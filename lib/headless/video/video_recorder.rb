...
class Headless
  class VideoRecorder
    ...
    def start_capture
      audio = @audio ? "-f alsa -ac 2 -i pulse" : ""
      CliUtil.fork_process("#{CliUtil.path_to('ffmpeg')} -y -r #{@frame_rate} -s #{@dimensions} -f x11grab -i :#{@display} -vcodec #{@codec} #{audio} #{@tmp_file_path}", @pid_file_path, @log_file_path)
      at_exit do
